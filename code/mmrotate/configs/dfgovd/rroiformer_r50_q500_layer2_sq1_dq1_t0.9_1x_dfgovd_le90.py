_base_ = [
    '../_base_/datasets/dfgovd.py', '../_base_/schedules/schedule_1x.py',
...
model = dict(
    ...
    roi_head=dict(
        type='RRoIFormerDecoder',
        num_stages=2,
        stage_loss_weights=[1] * 2,
        selective_query=[0],    # todo: note the selective queries stage
        distinct_query=0,       # todo: note the distinct queries stage
        distinct_cfg=dict(type='nms_rotated', iou_threshold=0.9),
    ...
    train_cfg=dict(
        ...
        rcnn=[
            dict(
                ...
                pos_weight=1) for _ in range(2)
        ]),
    test_cfg=dict(
        rpn=dict(),
        rcnn=dict(max_per_img=500)))
